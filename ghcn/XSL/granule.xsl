...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:variable name="collDimensions"
    select='$coll//gmd:axisDimensionProperties/gmd:MD_Dimension[gmd:dimensionName/gmd:MD_DimensionNameTypeCode/@codeList = "http://www.isotc211.org/2005/resources/Codelist/gmxCodelists.xml#MD_DimensionNameTypeCode"]'/>
  ...
  <xsl:template match='gmd:resolution[@gco:nilReason = "missing"]'>
    ...
    <xsl:variable name="dimensionName"
      select='../gmd:dimensionName/gmd:MD_DimensionNameTypeCode[@codeList = "http://www.isotc211.org/2005/resources/Codelist/gmxCodelists.xml#MD_DimensionNameTypeCode"]/text()'/>

    <!-- see if there's a matching dimension in the collection. -->
    <xsl:choose>
      <xsl:when
        test="$collDimensions[gmd:dimensionName/gmd:MD_DimensionNameTypeCode/text() = $dimensionName]">
        <!-- found a dimension with the right name in collection; copy it out of the collection. -->
        <xsl:apply-templates
          select="$collDimensions[gmd:dimensionName/gmd:MD_DimensionNameTypeCode/text() = $dimensionName]/gmd:resolution"
          mode="collection"/>
      </xsl:when>
      <xsl:otherwise>
