...
<!--NCEI-MD added for extra service links -->
	...
	<xsl:variable name="Pathfinderopendap" select="concat('http://data.nodc.noaa.gov/thredds/dodsC/', netcdf/path,netcdf/title,'.nc.html')"/>
...
<!--Need to check with Pathfinder-->
	...
	<xsl:variable name="creatorDate" select="netcdf/attribute[@name='creation_date']/@value"/>
			...
			<xsl:if test="$Pathfinderthredds">
				...
				<!-- WMS -->
			        <xsl:call-template name="writeService">
			          ...
			          <xsl:with-param name="operationURL" select="concat('http://data.nodc.noaa.gov/thredds/wms/', netcdf/path, netcdf/title,'.nc?service=WMS&amp;version=1.3.0&amp;request=GetCapabilities')"/>
			        </xsl:call-template>
			     ...
			     <!-- WCS -->
			        <xsl:call-template name="writeService">
			          ...
			          <xsl:with-param name="serviceOperationName" select="'GetCapabilities'"/>
			          <xsl:with-param name="operationURL" select="concat('http://data.nodc.noaa.gov/thredds/wcs/', netcdf/path, netcdf/title,'.nc?service=WCS&amp;version=1.0.0&amp;request=GetCapabilities')"/>
			        </xsl:call-template>
